...
{
// DATA
 TFile *f0 = new TFile("../output/Data_SSVHEM.root");
// Primary MC sample
 TFile *f1 = new TFile("../output/MC_QCD-Pt153080_SSVHEM.root");
// MC with Track History
 TFile *f2 = new TFile("../output/MC_QCD-Pt153080_SSVHEM.root");
...
//$$
  ...
  TLegend* leg = new TLegend(0.22,0.70,0.62,0.93);
    ...
    leg->Draw();
       ...
       float Mis50 = Fun->Integral(49,51)/2.;
       float Mis50Err = (Gun->Integral(49,51)/2. - Hun->Integral(49,51)/2.) / 2;

       float SF50 = FUn->Integral(49,51)/2.;
       float SF50Err = (GUn->Integral(49,51)/2. - HUn->Integral(49,51)/2.) / 2;

cout << endl;
cout << " At pt = 50 GeV: Mistag = " << Mis50 << " +_ " << Mis50Err
     << "   SF = " << SF50 << " +_ " << SF50Err << endl;
cout << endl;
