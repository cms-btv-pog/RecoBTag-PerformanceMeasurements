...
TString adddir = "./";
TString filename_1=adddir+"QCD_8P6NoPU.root";
TString labelfile1="QCD 8TeV Pyth8 No PU";
TString filename_2=adddir+"QCD_8P6NoPU.root";
...
TString filename_4=adddir+"QCD_8P6NoPU.root";
TString labelfile4="QCD 8TeV Pyth6 w/ PU";


bool boolfile5=false;
TString filename_5=adddir+"QCD_8P6NoPU.roott";
TString labelfile5="QCD 13TeV Pyth8 No PU";

bool boolfile6=false;
TString filename_6=adddir+"QCD_8P6NoPU.root";
...
TString filename = "QCD_8P6NoPU.root";
bool btag = false;


TString titlemc = "CMS Simulation";
...
void DrawDataMC(TString name, TString histotitle, bool log);
...
void DrawCommPlot(bool Draw_track_plots, bool Draw_Nminus1_plots, bool Draw_sv_plots, bool Draw_muons_plots, bool Draw_discriminator_plots , 
  ...
  DrawDataMC("jet_pt_all"   ,"pT of all jets",1);	    
  DrawDataMC("jet_eta"      ,"eta of all jets",	0);    
  DrawDataMC("jet_phi"      ,"phi of all jets",0);
  DrawDataMC("track_multi"  ,"number of tracks in the jets", 0);
  //DrawDataMC("nPV"      ,"# of PV",0);

  //cout << "Drawing tack plots\n";
  if (Draw_track_plots){	      
    DrawDataMC("track_multi"  ,      "number of tracks in the jets",0);		   
    DrawDataMC("trk_multi_sel"  ,    "number of selected tracks in the jets",0);	   
    DrawDataMC("track_chi2"   ,      "normalized #chi^{2} of the tracks"	,1);	   
    DrawDataMC("track_nHit" ,      "number of hits",1);		   
    DrawDataMC("track_HPix"   ,      "number of hits in the Pixel",1);		   
    DrawDataMC("track_len"     ,     "Track decay length",1);				   
    DrawDataMC("track_dist"    ,     "Track distance to the jet axis"	,1);	   
    DrawDataMC("track_dz"     ,      "Track IP_dz",1);				   
    DrawDataMC("track_pt"     ,      "pT of all the tracks",1);			   
    DrawDataMC("track_pt15"     ,      "pT of all the tracks",1);			   
    //Draw("track_isfromSV",     "Track is from SV",1);			   

    DrawDataMC("track_IPs"    ,      "3D IP significance of all tracks",1);	   
    DrawDataMC("track_IPs1tr" ,      "3D IP significance of the first track",1);	   
    DrawDataMC("track_IPs2tr" ,      "3D IP significance of the second track",1);     
    DrawDataMC("track_IPs3tr" ,      "3D IP significance of the third track",1);	
    DrawDataMC("track_IP"     ,      "3D IP of all tracks",1);			   
    DrawDataMC("track_IP1tr"  ,      "3D IP of the first track"	,1);	   
    DrawDataMC("track_IP2tr"  ,      "3D IP of the second track",1);		   
    DrawDataMC("track_IP3tr"  ,      "3D IP of the third track"	,1);	
    DrawDataMC("track_IP2Ds"	,    "2D IP significance of all tracks"	,1);   
    DrawDataMC("track_IP2Ds1tr" ,    "2D IP significance of the first track",1);	   
    DrawDataMC("track_IP2Ds2tr" ,    "2D IP significance of the second track" ,1);    
    DrawDataMC("track_IP2Ds3tr" ,    "2D IP significance of the third track" ,1);
    DrawDataMC("track_IP2D"    ,     "2D IP of all tracks",1);			   
    DrawDataMC("track_IP2D1tr" ,     "2D IP of the first track"	,1);	   
    DrawDataMC("track_IP2D2tr" ,     "2D IP of the second track",1);		   
    DrawDataMC("track_IP2D3tr" ,     "2D IP of the third track"	,1);	
    DrawDataMC("track_IP2Derr" ,     "2D IP error of all tracks",1);				   
    DrawDataMC("track_IP2Derr1tr" ,  "2D IP error of the first track",1);  	   
    DrawDataMC("track_IP2Derr2tr" ,  "2D IP error of the second track" ,1);	   
    DrawDataMC("track_IP2Derr3tr" ,  "2D IP error of the third track",1);  	
    DrawDataMC("track_IPerr"   ,     "3D IP error of all tracks",1);				   
    DrawDataMC("track_IPerr1tr"   ,  "3D IP error of the first track" ,1); 	   
    DrawDataMC("track_IPerr2tr"   ,  "3D IP error of the second track" ,1);	   
    DrawDataMC("track_IPerr3tr"   ,  "3D IP error of the third track" ,1); 	

  }  
  if (Draw_Nminus1_plots){  
    DrawDataMC("track_chi2_cut"    ,"Normalized #chi^{2} @N-1 step",1);	    
    DrawDataMC("track_nHit_cut"  ," Number of hits @N-1 step",1);
    DrawDataMC("track_HPix_cut"    ,"Number of hits in the Pixel @N-1 step",1);  
    DrawDataMC("track_len_cut"     ,"Decay length @N-1 step",1);		    
    DrawDataMC("track_dist_cut"    ,"Distance to the jet axis @N-1 step" ,1);  
    DrawDataMC("track_dz_cut"     , "Transverse IP @N-1 step",1);		    
    DrawDataMC("track_pt_cut"	   ,"Track pT @N-1 step",1);
    DrawDataMC("track_pt15_cut"	   ,"Track pT @N-1 step",1);
  }
  if (Draw_sv_plots){

    DrawDataMC("jet_pt_sv"    ,"pT of jets containing a SV",1);
    DrawDataMC("sv_multi_0","nr. of SV including bin 0",1); 
    DrawDataMC("sv_multi","nr. of SV",1);
    DrawDataMC("sv_mass","SV mass",0);
    DrawDataMC("sv_mass_3trk","SV mass if #tracks@SV >=3",0);
    DrawDataMC("sv_chi2norm","SV norm. #chi^{2}",1);
    DrawDataMC("sv_deltaR_jet","Delta R between the jet and the SV direction.",0);
    DrawDataMC("sv_deltaR_sumJet","#Delta R between the jet and the SV {p}",0);
    DrawDataMC("sv_deltaR_sumDir","#Delta R between the SV direction and the SV {p}",0);
    DrawDataMC("sv_en_ratio","SV energy ratio",0);	
    DrawDataMC("sv_aboveC","IP2D of the first track above the charm threshold",1);	
    DrawDataMC("sv_pt","SV p_{T}",1); 	
    DrawDataMC("sv_eta","SV #eta",0);	
    DrawDataMC("sv_phi","SV #phi",0);	
    DrawDataMC("sv_flight3D","SV 3D flight distance",1);	
    DrawDataMC("sv_flight2D","SV 2D flight distance",1);	
    DrawDataMC("sv_flight2D_3trk","SV 2D flight dist. if #tracks@SV >=3",1);	
    DrawDataMC("sv_flight2DSig_3trk","SV 2D flight dist. sig. if #tracks@SV >=3",1);	
    DrawDataMC("sv_flight3DSig","SV 3D flight distance significance",1);
    DrawDataMC("sv_flightSig2D","SV 2D flight distance significance",1);
    DrawDataMC("sv_flight3Derr","SV 3D flight distance error",1);
    DrawDataMC("sv_flight2Derr","SV 2D flight distance error",1);
    DrawDataMC("svnTrk","nr. of tracks from a SV",1);	
    DrawDataMC("svnTrk_firstVxt","nr. of tracks from the first SV",1);
  }
  if (Draw_muons_plots){  
    DrawDataMC("muon_multi"   ,      "number of muons", 1);       
    DrawDataMC("muon_multi_sel"   ,  "number of selected muons",1);
    DrawDataMC("mu_ptrel"     ,      "p_{T} rel. of the muon",0);    
    DrawDataMC("mu_chi2"      ,      "norm. #chi^{2} of the muon", 1);
    DrawDataMC("muon_Pt",	     "Muon p_{T}",1);	       
    DrawDataMC("muon_eta",	     "Muon #eta",0);	       
    DrawDataMC("muon_phi",	     "Muon #phi",0);	       
    DrawDataMC("muon_Ip3d",	     "Muon 3D IP",1);	       
    DrawDataMC("muon_Ip2d",	     "Muon 2D IP",1);	       
    DrawDataMC("muon_Sip3d",	     "Muon 3D IP significance",1);
    DrawDataMC("muon_Sip2d",	     "Muon 2D IP significance",1);
    DrawDataMC("muon_DeltaR",	     "Muon1 #Delta R",0);

  } 
  if (Draw_discriminator_plots){  
    DrawDataMC("TCHE_extended1"       ,"TCHE (extended)",1);    
    DrawDataMC("TCHP_extended1"       ,"TCHP (extended)",1);    
    DrawDataMC("discri_ssche0",      "SSVHE Discriminator", 1);
    DrawDataMC("discri_sschp0",      "SSVHP Discriminator", 1);

    DrawDataMC("TCHE"	      ,"TCHE Discriminator", 1); 		    
    DrawDataMC("TCHP"	      ,"TCHP Discriminator",1);  		    
    DrawDataMC("JP"	      ,"JP Discriminator",1);			    
    DrawDataMC("JBP"	      ,"JBP Discriminator",1);			    
    DrawDataMC("SSV"	      ,"SSVHE Discriminator",0);			    
    DrawDataMC("SSVHP"        ,"SSVHP Discriminator",0); 		    
    DrawDataMC("CSV"	      ,"CSV Discriminator",1);

  }
  if (Draw_newdiscriminator_plots) {
    DrawDataMC("RetCombSvx"         ,"CSV_v1 Discriminator",1);
    DrawDataMC("RetCombSvx"         ,"CSV_v1 Discriminator",0);
    DrawDataMC("CombCSVJP"          ,"CSVJP Discriminator",1);
    DrawDataMC("CombCSVSL"          ,"CSVSL_v1 Discriminator",1);
    DrawDataMC("CombCSVJPSL"        ,"CSVJPSL Discriminator",1);
    DrawDataMC("SoftMu"        ,"Soft Muon Discriminator",1);
    DrawDataMC("SoftMu"        ,"Soft Muon Discriminator",0);
    DrawDataMC("SoftEl"        ,"Soft Electron Discriminator",1);
  }
  ...
  if (Draw_pflepton) {
    ...
    DrawDataMC("pfelectron_multi","number of pfelectron",1);
    DrawDataMC("pfelectron_pt","pfelectron p_{T}",1);
    DrawDataMC("pfelectron_eta","pfelectron #eta",0);
    DrawDataMC("pfelectron_phi","pfelectron #phi", 0);
    DrawDataMC("pfelectron_Sip","3D IP significance of pfelectron",1);
    DrawDataMC("pfelectron_ptrel","pT rel. of the pfelectron",0);
    DrawDataMC("pfelectron_ratio","ratio of pfelectron", 1);
    DrawDataMC("pfelectron_ratiorel",   "ratioRel of pfelectron",1);
    DrawDataMC("pfelectron_deltar", "#DeltaR(pfelectron,jet)",0);

    DrawDataMC("pfmuon_multi","number of pfmuon",1);
    DrawDataMC("pfmuon_goodquality","quality of pfmuons",0);
    DrawDataMC("pfmuon_pt","pfmuon p_{T}",1);
    DrawDataMC("pfmuon_eta","pfmuon #eta",0);
    DrawDataMC("pfmuon_phi","pfmuon #phi", 0);
    DrawDataMC("pfmuon_Sip","3D IP significance of pfmuon",1);
    DrawDataMC("pfmuon_ptrel","pT rel. of the pfmuon",0);
    DrawDataMC("pfmuon_ratio","ratio of pfmuon", 1);
    DrawDataMC("pfmuon_ratiorel",   "ratioRel of pfmuon",1);
    DrawDataMC("pfmuon_deltar", "#DeltaR(pfmuon,jet)",0);
...
void DrawMC(TString name, TString histotitle, bool log, double sigma, float intL=1000.) {
