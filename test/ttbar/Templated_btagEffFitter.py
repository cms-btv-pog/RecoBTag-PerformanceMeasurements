...
def prepareTemplates(tagger,taggerDef,var,varRange,channelList,inDir,outDir):
    ...
    passSlice = array( 'i',len(SLICEBINS)*[ 0 ] )
    outT.Branch( 'slice', passSlice, 'slice[%d]/I'%len(SLICEBINS) )
        ...
        for i in xrange(0,chains[key].GetEntries()):
            ...
            for systVar in SYSTVARS:
                ...
                for islice in xrange(0,len(passSlice)):
                    ...
                    histos[hkey].Fill(varVal[0],weight)
                    for iop in xrange(2,len(taggerDef)-1):
                        if taggerVal[0]< taggerDef[iop] : continue
