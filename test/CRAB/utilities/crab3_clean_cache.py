#!/usr/bin/env python
"""Description: 
 script to clean CRAB cache area

 * the script requires a valid proxy and the CRAB environment
 * if the environment variable X509_USER_PROXY is not set, create it manually via

   $ export X509_USER_PROXY=[path to grid proxy]

 * REF: https://twiki.cern.ch/twiki/bin/view/CMSPublic/CRAB3FAQ#How_can_I_clean_my_area_in_the_C
"""
...
if __name__ == '__main__':
    ...
    if 'X509_USER_PROXY' not in os.environ:
       log  = '\n > ERROR -- '
       log += 'enviroment variable X509_USER_PROXY not defined'
       log += ', create it (with path to grid proxy) before running this script'
       log += '\n\n example: "export X509_USER_PROXY=/tmp/x509up_XXXXXX"\n'
