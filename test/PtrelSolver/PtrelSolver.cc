...
void PtrelSolver::measure(const char *inputfilename, const char *dir, const char *outfilename, const char *tag, const char *pthist, const char *etahist, bool sys) {
  ...
  name.Resize(0); name.Append(dir); name.Append("/"); name.Append(pthist);
  hist2 = (TH2F *)inputfile->Get(name.Data());

  tag_name.Resize(0); 
  tag_name.Append(pthist); tag_name.Insert(1, "tag"); 
  tag_name.Append("_"); tag_name.Append(tag);
  tag_name.Insert(0, "/"); tag_name.Insert(0, dir);
  hist2_tag = (TH2F *)inputfile->Get(tag_name.Data());
  ...
  eff_name.Resize(0);
  eff_name.Append(tag); eff_name.Append("_");
  eff_name.Append(pthist);
  eff = (TH1F *)hist2->ProjectionX(eff_name.Data());
  eff_name_sys.Append(eff_name.Data());
  eff_name_sys.Append("_sys");
  eff_sys = (TH1F *)hist2->ProjectionX(eff_name_sys.Data());
  ...
  for (int ii = 0; ii <= nbins; ii++) {
    ...
    // before tag
    sprintf(data_name, "%s_ptbin_%d", tag, ii);
    if (ii !=0) {
    ...
    // after tag
    sprintf(data_name, "%s_ptbin_%d_tag", tag, ii);
    if (ii !=0) {
    ...
    if (ii == 0) {
      std::cout <<  "information: tagger[ " << tag << "] average efficiency " 
		<< result[0] <<"+/-" 
		<< result[1] <<"(stat.)+/-";
  ...
  mc_eff = getMCeff(inputfile, dir, pthist, tag);
  ...
  // 2. eta dependence
  // duplicate the binning for efficiency table
  name.Resize(0); name.Append(dir); name.Append("/"); name.Append(etahist);
  hist2 = (TH2F *)inputfile->Get(name.Data());

  tag_name.Resize(0); 
  tag_name.Append(etahist); tag_name.Insert(1, "tag"); 
  tag_name.Append("_"); tag_name.Append(tag);
  tag_name.Insert(0, "/"); tag_name.Insert(0, dir);
  hist2_tag = (TH2F *)inputfile->Get(tag_name.Data());
  ...
  mc_eff = getMCeff(inputfile, dir, etahist, tag);
  hist = Divide(eff, mc_eff);
...
void PtrelSolver::measure(const char *inputfilename, const char *outputfilename, const char *tag, const char *pthist, const char *etahist, bool sys) {

  this->measure(inputfilename, "/Histograms/muon_in_jet/", outputfilename, tag, pthist, etahist, sys);
}
