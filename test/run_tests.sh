...
cmsRun runBTagAnalyzer_cfg.py defaults=Commissioning18 runOnData=False maxEvents=20 miniAOD=False inputFiles=/store/mc/RunIIFall17DRPremix/QCD_Pt_50to80_TuneCP5_13TeV_pythia8/AODSIM/94X_mc2017_realistic_v10-v1/60000/2AE14B27-6FE7-E711-B564-00266CF94C44.root useSelectedTracks=False produceJetTrackTree=True runCTagVariables=False &> last.log || die $?
...
cmsRun runBTagAnalyzer_cfg.py defaults=Commissioning18 useSelectedTracks=False produceJetTrackTree=True runOnData=True maxEvents=20 &> last.log || die $?

echo 'Running on MC -- commissioning'
cmsRun runBTagAnalyzer_cfg.py defaults=Commissioning18 useSelectedTracks=False produceJetTrackTree=True runOnData=False maxEvents=20 &> last.log || die $?

echo 'Running on data -- ctag'
cmsRun runBTagAnalyzer_cfg.py defaults=Commissioning18 runOnData=True maxEvents=20 runCTagVariables=True runFatJets=True runSubJets=True &> last.log || die $?

echo 'Running on MC -- ctag'
cmsRun runBTagAnalyzer_cfg.py defaults=Commissioning18 runOnData=False maxEvents=20 runCTagVariables=True runFatJets=True runSubJets=True &> last.log || die $?
