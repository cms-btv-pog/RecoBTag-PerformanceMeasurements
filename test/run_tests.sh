...
cmsRun runBTagAnalyzer_cfg.py defaults=Moriond18 runOnData=True maxEvents=20 &> last.log || die $?
...
cmsRun runBTagAnalyzer_cfg.py defaults=Moriond18 runOnData=False maxEvents=20 runFatJets=True &> last.log || die $?

echo 'Running on data -- commissioning'
cmsRun runBTagAnalyzer_cfg.py defaults=Moriond18 useSelectedTracks=False fillsvTagInfo=True produceJetTrackTree=True runOnData=True maxEvents=20 &> last.log || die $?

echo 'Running on MC -- commissioning'
cmsRun runBTagAnalyzer_cfg.py defaults=Moriond18 useSelectedTracks=False fillsvTagInfo=True produceJetTrackTree=True runOnData=False maxEvents=20 &> last.log || die $?

echo 'Running on data -- ctag'
cmsRun runBTagAnalyzer_cfg.py defaults=Moriond18 runOnData=True maxEvents=20 doCTag=True runFatJets=True runSubJets=True &> last.log || die $?

echo 'Running on MC -- ctag'
cmsRun runBTagAnalyzer_cfg.py defaults=Moriond18 runOnData=False maxEvents=20 doCTag=True runFatJets=True runSubJets=True &> last.log || die $?
