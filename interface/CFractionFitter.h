// @(#)root/hist:$Id: CFractionFitter.h,v 1.1 2009/05/18 17:24:26 mapinas Exp $
// Author: Frank Filthaut filthaut@hef.kun.nl  20/05/2002

#ifndef ROOT_CFractionFitter
#define ROOT_CFractionFitter
...
class CFractionFitter: public TObject
{
public:
    CFractionFitter();
    CFractionFitter(TH1* data, TObjArray *MCs);
    virtual ~CFractionFitter();
    ...
    Int_t Fit(Double_t errDef = 0.5);

    void GetResult(Int_t parm, Double_t& value, Double_t& error) const;
    TH1* GetPlot();

    // This global function needs access to computeFCN()
    friend void CFractionFitFCN(Int_t& npar, Double_t* gin, Double_t& f, Double_t* par, Int_t flag);
...
protected:
    ...
    //ClassDef(CFractionFitter, 0)   // Fits MC fractions to data histogram
};

//
//  CFractionFitFCN
//
//  Computes negative log-likelihood for CFractionFitter
//

void CFractionFitFCN(Int_t& npar, Double_t* gin, Double_t& f, Double_t* par, Int_t flag);

#endif // ROOT_CFractionFitter
