// @(#)root/hist:$Id: CFractionFitter.cc,v 1.1 2009/05/18 17:24:26 mapinas Exp $
...
//   mc->Add(mc1);
//   mc->Add(mc2);
//   CFractionFitter* fit = new CFractionFitter(data, mc); // initialise
...
//     CFractionFitter* fit = new CFractionFitter(data, mc);
...
//ClassImp(CFractionFitter)

//______________________________________________________________________________
CFractionFitter::CFractionFitter() :
        fFitDone(kFALSE),fData(0), fPlot(0)
{
    // CFractionFitter default constructor.
...
//______________________________________________________________________________
CFractionFitter::CFractionFitter(TH1* data, TObjArray  *MCs) :
        fFitDone(kFALSE), fChisquare(0), fPlot(0)
{
    // CFractionFitter constructor. Does a complete initialisation (including
    ...
    fractionFitter->SetFCN(CFractionFitFCN);
...
CFractionFitter::~CFractionFitter()
{
    // CFractionFitter default destructor
...
//______________________________________________________________________________
void CFractionFitter::SetData(TH1* data)
{
    ...
    //   rather instantiate a new CFractionFitter object)
...
//______________________________________________________________________________
void CFractionFitter::SetMC(Int_t parm, TH1* MC)
{
...
//______________________________________________________________________________
void CFractionFitter::SetWeight(Int_t parm, TH1* weight)
{
...
//______________________________________________________________________________
TVirtualFitter* CFractionFitter::GetFitter() const
{
...
//______________________________________________________________________________
void CFractionFitter::CheckParNo(Int_t parm) const
{
...
//______________________________________________________________________________
void CFractionFitter::SetRangeX(Int_t low, Int_t high)
{
...
//______________________________________________________________________________
void CFractionFitter::ReleaseRangeX()
{
...
//______________________________________________________________________________
void CFractionFitter::SetRangeY(Int_t low, Int_t high)
{
...
//______________________________________________________________________________
void CFractionFitter::ReleaseRangeY()
{
...
//______________________________________________________________________________
void CFractionFitter::SetRangeZ(Int_t low, Int_t high)
{
...
//______________________________________________________________________________
void CFractionFitter::ReleaseRangeZ()
{
...
//______________________________________________________________________________
void CFractionFitter::Constrain(Int_t parm, Double_t low, Double_t high)
{
...
//______________________________________________________________________________
void CFractionFitter::UnConstrain(Int_t parm)
{
...
//______________________________________________________________________________
void CFractionFitter::CheckConsistency()
{
...
//______________________________________________________________________________
Int_t CFractionFitter::Fit(Double_t errDef)
{
    // Perform the fit with the default UP value.
    // The value returned is the minimisation status.

    //Double_t	printLevel	=	-1;
    //fractionFitter->ExecuteCommand("SET PRINT",&printLevel,1);
...
//______________________________________________________________________________
void CFractionFitter::ErrorAnalysis(Double_t UP)
{
...
//______________________________________________________________________________
void CFractionFitter::GetResult(Int_t parm, Double_t& value, Double_t& error) const
{
...
//______________________________________________________________________________
void CFractionFitter::GetRanges(Int_t& minX, Int_t& maxX, Int_t& minY, Int_t& maxY,
...
//______________________________________________________________________________
void CFractionFitter::ComputeFCN(Int_t& /*npar*/, Double_t* /*gin*/,
                                 Double_t& f, Double_t* xx, Int_t flag)
{
    ...
    if (flag == 3)
    {
        TString ts = "Fraction fit to hist: ";
        ts += fData->GetName();
            ...
            for (x = minX; x <= maxX; ++x)
            {
                ...
                for (mc = 0; mc < fNpar; ++mc)
                {
                    ...
                    if (binContent > 0 && binPrediction > 0)
                    {
                        ...
                        result -= binContent*TMath::Log(binContent);	//SAK
                        result += binContent;												//SAK
                    }
                ...
                if (found > 0 && prediction > 0)
                {
                    ...
                    result -= found*TMath::Log(found);			//SAK
                    result += found;												//SAK
                }
            }
        }
...
//______________________________________________________________________________
void CFractionFitter::FindPrediction(int bin, Double_t *fractions, Double_t &ti, int& k0, Double_t &aki) const
{
    ...
    // Sanity check: none of the fractions should be =0 -- SAK : no clue why, so skip
    for (par = 0; par < fNpar; ++par)
    {
...
#ifdef __DONT_THINK_SO__
            return;
#endif //__DONT_THINK_SO__
        }
    }
    ...
    for (Int_t iteration = 0; iteration < 100; ++iteration)
    {
        ...
        for (par = 0; par < fNpar; ++par)
        {
            ...
            Double_t weightedFraction = hw ?
                                        hw->GetBinContent(bin) * fractions[par] : fractions[par];
            //SAK// Double_t d = 1/(ti+1/weightedFraction);
            Double_t d = weightedFraction ? 1/(ti+1/weightedFraction) : 0;
            aFunction   += h->GetBinContent(bin)*d;
            aDerivative -= h->GetBinContent(bin)*d*d;
        }
        if (TMath::Abs(aFunction) < 1e-12) break;
        //-- SAK ----------------------------------------------------------------
        if (aDerivative == 0)  	// Out, damned spot
        {
            step /= 10;
            ti = 0;
        }
...
//______________________________________________________________________________
void CFractionFitFCN(Int_t& npar, Double_t* gin, Double_t& f, Double_t* par, Int_t flag)
{
    // Function called by the minimisation package. The actual functionality is passed
    // on to the CFractionFitter::ComputeFCN member function.

    CFractionFitter* fitter = dynamic_cast<CFractionFitter*>(fractionFitter->GetObjectFit());
    if (!fitter)
    {
        fitter->Error("CFractionFitFCN","Invalid fit object encountered!");
        return;
    }
...
//______________________________________________________________________________
Double_t CFractionFitter::GetChisquare() const
{
...
//______________________________________________________________________________
Int_t CFractionFitter::GetNDF() const
{
...
//______________________________________________________________________________
Double_t CFractionFitter::GetProb() const
{
...
//______________________________________________________________________________
void CFractionFitter::ComputeChisquareLambda()
{
...
//______________________________________________________________________________
TH1* CFractionFitter::GetMCPrediction(Int_t parm) const
{
