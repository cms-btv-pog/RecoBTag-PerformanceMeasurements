...
PerformanceAnalyzer::analyze(const Event& iEvent, const EventSetup& iSetup)
{
    ...
    for ( jet = recoJets.begin(); jet != recoJets.end(); ++jet )
    {
        ...
        if ( bTagTrackEventIPTagInfos.isValid() )
        {
            ...
            std::vector<Measurement1D> ip3ds( (*tagInfo)[jetIndex].impactParameters(0) );
            std::vector<Measurement1D> ip2ds( (*tagInfo)[jetIndex].impactParameters(1) );
            std::vector<Measurement1D> sdls( (*tagInfo)[jetIndex].decayLengths() );
            std::vector<Measurement1D> dtas( (*tagInfo)[jetIndex].jetDistances() );
