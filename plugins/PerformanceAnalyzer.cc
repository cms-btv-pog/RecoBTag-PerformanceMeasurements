...
PerformanceAnalyzer::analyze(const Event& iEvent, const EventSetup& iSetup)
{
  ...
  for( jet = recoJets.begin(); jet != recoJets.end(); ++jet ) {
    ...
    for (size_t k=0; k<jetTags_testManyByType.size(); k++) {
      ...
      //std::cout << "ith_tagged   " << ith_tagged << std::endl;
	...
	fS8evt->btag_NegTag_disc3D_2trk.push_back( (*jetTags)[ith_tagged].discriminator() ); // 3rd trk, 3D
    ...
    //drop the following lines ? Is this necessary?
